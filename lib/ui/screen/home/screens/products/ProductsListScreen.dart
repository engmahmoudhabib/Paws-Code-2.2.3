...
class ProductsListScreen extends StatefulWidget {
  ProductsListScreen(this.showServiceDetails, this.favorite, {this.initialId});
...
class _ProductsListScreenState extends BaseStatefulState<ProductsListScreen> {
  ...
  @override
  void initState() {
    context.read(productsProvider.notifier).animalType = null;
    context.read(productsProvider.notifier).tags = null;
    Future.delayed(Duration.zero, () {
      context.read(productsProvider.notifier).load(favorite: widget.favorite);
    });
    try {
        ...
        children: [
          widget.favorite.length == 0
              ? Column(
                  children: [
                    SizedBox(
                      height: widget.showServiceDetails.value ? 0 : 20.h,
                    ),
                    !widget.showServiceDetails.value
                        ...
                        : Container(),
                    SizedBox(
                      height: widget.showServiceDetails.value ? 0 : 10.h,
                    ),
                  ],
                )
              : Container(),
              ...
              : Expanded(
                  child: Consumer(
                    builder: (context, watch, child) {
                      final response = watch(productsProvider);
                      return response.when(idle: () {
                        ...
                        try {
                          ProductsResponse data = ProductsResponse.fromMap(map);
  ...
  getCategories() {
    try {
      ...
      t.insert(0, {
        'id': 'all',
        'margin-left': 48.w,
        'selected': context.read(productsProvider.notifier).tags == null,
                      ...
                      InkWell(
                        ...
                        child: Padding(
                          ...
                          child: favorite.length > 0
                              ? Container()
                              : item.userReactions?.isFavorited ?? false
                                  ? SvgPicture.string(
                      ...
                      Flexible(
                        ...
                        child: Row(
                          children: [
                            ...
                            Flexible(
                              child: TextResponsive(
                                'السعر:${item.price?.formatted?.toString()}',
                  ...
                  Row(
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          borderRadius:
                              BorderRadius.all(Radius.elliptical(3.5.w, 4.0.w)),
                      ...
                      RatingBar(
                        initialRating:
                            double.parse(item.stats!.reviewsAvg ?? '0.0'),
                        minRating: 5,
